...
personal_accounts: AccountsList = account_factory.get_account_from_group(group=AccountFactory.AccountGroups.PERSONAL)
ordial_accounts: AccountsList = account_factory.get_account_from_group(group=AccountFactory.AccountGroups.ORDIAL)
...
if __name__ == "__main__":
    ...
    #         (all_accounts['OrdialNationalBankOP'], MeanTransactionsStrategy(), pred_args),
    #         (all_accounts['OrdialNationalBankCC'], MeanTransactionsStrategy(), pred_args),
    ...
    for i in range(0, 7):
        dd = start_date - dt.timedelta(days=1*i*4)
        start_date = dt.date(dd.year, dd.month, 1)
        ordial_accounts.barplot(dd.strftime('%Y-%m'), fig_name='Ordial')

    ordial_accounts.plot(fig_name='Ordial Accounts')
